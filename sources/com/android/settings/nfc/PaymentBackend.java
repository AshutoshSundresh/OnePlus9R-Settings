...
public class PaymentBackend {
    ...
    public void refresh() {
        ...
        if (fragment2 == null || (fragment2.getActivity() != null && this.fragment.isAdded())) {
            ...
            if (OPUtils.isSupportNfcUicc() && !arrayList.isEmpty()) {
                arrayList = addUICCAppInfo(arrayList);
                PaymentAppInfo defaultUiccPaymentApp = getDefaultUiccPaymentApp();
                if (defaultUiccPaymentApp != null) {
                    paymentAppInfo = defaultUiccPaymentApp;
                }
            }
            this.mAppInfos = arrayList;
            Fragment fragment3 = this.fragment;
            if (fragment3 != null && (fragment3 instanceof PaymentSettings)) {
                if (arrayList.isEmpty()) {
                    ((PaymentSettings) this.fragment).getEmptyView().setVisibility(0);
                } else {
                    ((PaymentSettings) this.fragment).getEmptyView().setVisibility(8);
                }
            }
    ...
    private ArrayList<PaymentAppInfo> addUICCAppInfo(ArrayList<PaymentAppInfo> arrayList) {
        ArrayList<PaymentAppInfo> arrayList2 = new ArrayList<>();
        List<PaymentAppInfo> sEList = NfcUiccUtils.getSEList(this.mContext);
        if (sEList != null && !sEList.isEmpty()) {
            arrayList2.addAll(sEList);
        }
    ...
    /* access modifiers changed from: package-private */
    public PaymentAppInfo getDefaultUiccPaymentApp() {
        return NfcUiccUtils.getUICCDefaultAppInfo(this.mContext);
    }
