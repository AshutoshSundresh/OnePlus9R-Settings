...
public class InteractAcrossProfilesDetails extends AppInfoBase implements Preference.OnPreferenceClickListener {
    ...
    private void logEvent(int i) {
        if (RestrictedLockUtils.getProfileOrDeviceOwner(this.mContext, this.mWorkProfile) != null) {
    ...
    private void enableInteractAcrossProfiles(boolean z) {
        try {
            this.mCrossProfileApps.setInteractAcrossProfilesAppOp(this.mPackageName, z ? 0 : 1);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
