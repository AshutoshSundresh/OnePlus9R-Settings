...
public class AppDataUsagePreference extends AppPreference {
    ...
    public AppDataUsagePreference(Context context, AppItem appItem, int i, UidDetailProvider uidDetailProvider) {
        ...
        if (uidDetail != null) {
            lambda$new$0();
        } else if (!ProductUtils.isUsvMode() || appItem.key != 151) {
            ...
            UidDetail uidDetail2 = new UidDetail();
            this.mDetail = uidDetail2;
            uidDetail2.label = getContext().getString(C0017R$string.data_usage_video_call_title);
            this.mDetail.icon = getContext().getResources().getDrawable(C0008R$drawable.ic_video_call_icon);
            lambda$new$0();
        }
    }
