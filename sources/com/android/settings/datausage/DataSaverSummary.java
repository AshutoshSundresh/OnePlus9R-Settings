...
public class DataSaverSummary extends SettingsPreferenceFragment implements SwitchBar.OnSwitchChangeListener, DataSaverBackend.Listener, AppStateBaseBridge.Callback, ApplicationsState.Callbacks {
    ...
    public void onCreate(Bundle bundle) {
        ...
        this.mSM = (SubscriptionManager) this.mContext.getSystemService(SubscriptionManager.class);
        this.mTM = TelephonyManager.from(this.mContext);
    }
    ...
    public void onExtraInfoUpdated() {
        if (isAdded()) {
            ...
            for (int i2 = 0; i2 < size; i2++) {
                ...
                try {
                    if ((appEntry.extraInfo instanceof AppStateDataUsageBridge.DataUsageState) && ((AppStateDataUsageBridge.DataUsageState) appEntry.extraInfo).isDataSaverWhitelisted) {
                        i++;
                    }
                } catch (Exception e) {
                    e.printStackTrace();
    ...
    private void sendDataSaverInfo() {
        Intent intent = new Intent("sim.network.ACTION_CELLULAR_DATA_CHANGE");
        String networkOperator = this.mTM.getNetworkOperator();
        int defaultDataPhoneId = this.mSM.getDefaultDataPhoneId();
        long uTCTime = getUTCTime();
        intent.addFlags(16777216);
        intent.putExtra("switch_number", "4");
        intent.putExtra("data_saver_status", this.mStatus);
        intent.putExtra("slot_id", defaultDataPhoneId);
        intent.putExtra("mccmnc", networkOperator);
        intent.putExtra("current_time", uTCTime);
        Log.d("DataSaverSummary", "sendDataSaverInfo...status:" + this.mStatus + ",,,slotId:" + defaultDataPhoneId + ",,,mccmnc:" + networkOperator + ",,,current_time:" + uTCTime);
    ...
    private long getUTCTime() {
        ...
        Log.d("DataSaverSummary", "utc time:" + instance.getTimeInMillis());
