...
public class ChartDataUsagePreference extends Preference {
    ...
    public void setNetworkPolicy(NetworkPolicy networkPolicy) {
        ...
        if (networkPolicy != null) {
            int dataWarnState = OPDataUsageUtils.getDataWarnState(getContext(), this.mSubId);
            long dataWarnBytes = OPDataUsageUtils.getDataWarnBytes(getContext(), this.mSubId);
            if (dataWarnState == 1) {
                this.mPolicy.warningBytes = dataWarnBytes;
            } else {
                this.mPolicy.warningBytes = -1;
            }
            int dataLimitState = OPDataUsageUtils.getDataLimitState(getContext(), this.mSubId);
            long dataLimitBytes = OPDataUsageUtils.getDataLimitBytes(getContext(), this.mSubId);
            if (dataLimitState == 1) {
                this.mPolicy.limitBytes = dataLimitBytes;
            } else {
                this.mPolicy.limitBytes = -1;
            }
        }
        notifyChanged();
    }
    ...
    public void setNetworkCycleData(NetworkCycleChartData networkCycleChartData) {
        ...
        long[] dataUsageSectionTimeMillByAccountDay = OPDataUsageUtils.getDataUsageSectionTimeMillByAccountDay(getContext(), this.mSubId);
        setVisibleRange(dataUsageSectionTimeMillByAccountDay[0], dataUsageSectionTimeMillByAccountDay[1]);
        notifyChanged();
    }
