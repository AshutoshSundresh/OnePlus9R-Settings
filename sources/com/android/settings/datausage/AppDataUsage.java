...
public class AppDataUsage extends DataUsageBaseFragment implements Preference.OnPreferenceChangeListener, DataSaverBackend.Listener {
    ...
    public void onCreate(Bundle bundle) {
        ...
        if (i4 > 0) {
            if (!UserHandle.isApp(i4)) {
                ...
                if (ProductUtils.isUsvMode() && this.mAppItem.key == 151) {
                    uidDetail = new UidDetail();
                    uidDetail.label = getString(C0017R$string.data_usage_video_call_title);
                    uidDetail.icon = getResources().getDrawable(C0008R$drawable.ic_video_call_icon);
                }
                ...
                if (this.mPackages.size() != 0) {
                    try {
                        ...
                        try {
                            ...
                            drawable = packageManager.getApplicationInfo(applicationInfoAsUser.packageName, 0).loadIcon(packageManager);
                        } catch (PackageManager.NameNotFoundException e) {
                            e.printStackTrace();
                            drawable = null;
                        }
                        if (drawable == null) {
                            drawable = IconDrawableFactory.newInstance(getActivity()).getBadgedIcon(applicationInfoAsUser);
                        }
                        this.mIcon = getAdaptiveIconDrawable(drawable);
            ...
            while (true) {
                ...
                if (packageManager2.resolveActivity(this.mAppSettingsIntent, 0) != null) {
                    z = true;
                    break;
                }
            }
    ...
    private Drawable getAdaptiveIconDrawable(Drawable drawable) {
        try {
    ...
    public void bindData(int i) {
        ...
        long j3 = j2 + j;
        if (!ProductUtils.isUsvMode() || this.mAppItem.key != 151) {
            this.mTotalUsage.setSummary(DataUsageUtils.formatDataUsage(this.mContext, j3));
        ...
        long vtDataUsageBytes = DataUsageUtils.getVtDataUsageBytes();
        this.mTotalUsage.setSummary(DataUsageUtils.formatDataUsage(this.mContext, vtDataUsageBytes));
        this.mForegroundUsage.setSummary(DataUsageUtils.formatDataUsage(this.mContext, vtDataUsageBytes));
        this.mBackgroundUsage.setSummary(DataUsageUtils.formatDataUsage(this.mContext, 0));
    }
    ...
    class UpdateRuleTask extends AsyncTask<Void, Integer, Integer> {
        ...
        public void onPostExecute(Integer num) {
            ...
            if (this.progressDialog != null && AppDataUsage.this.isAdded()) {
