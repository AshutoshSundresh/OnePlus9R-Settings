...
public class NetworkDashboardFragment extends DashboardFragment implements MobilePlanPreferenceController.MobilePlanPreferenceHost, TetherDataObserver.OnTetherDataChangeCallback, Preference.OnPreferenceChangeListener {
    ...
    private OPRestrictedSwitchPreference mAirplaneModeRestrictedSwitchPreference;
    ...
    public void onStart() {
        ...
        if (ProductUtils.isUsvMode()) {
            OPRestrictedSwitchPreference oPRestrictedSwitchPreference = (OPRestrictedSwitchPreference) findPreference("airplane_mode");
            this.mAirplaneModeRestrictedSwitchPreference = oPRestrictedSwitchPreference;
            oPRestrictedSwitchPreference.setSummary(getContext().getResources().getString(C0017R$string.airplane_mode_description));
        }
    }
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        if (!"data_usage_summary".equals(preference.getKey()) || getActivity() == null || !getActivity().isInMultiWindowMode()) {
            return super.onPreferenceTreeClick(preference);
        }
        Toast.makeText(getPrefContext(), (CharSequence) getString(C0017R$string.feature_not_support_split_screen), 0).show();
        return true;
    }
