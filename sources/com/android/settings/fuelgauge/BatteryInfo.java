...
public class BatteryInfo {
    ...
    public void bindHistory(final UsageView usageView, BatteryDataParser... batteryDataParserArr) {
        ...
        usageView.setBottomLabels(new CharSequence[]{OPUtils.handleTimeSummary(string), OPUtils.handleTimeSummary(j != 0 ? context.getString(R$string.remaining_length_format, Formatter.formatShortElapsedTime(context, j / 1000)) : "")});
    }
    ...
    private static long getChargTimeToFull(Intent intent, BatteryStats batteryStats, long j) {
        ...
        if (OPUtils.isProductSwarpChargSupport()) {
            ...
            Log.d("BatteryInfo", "getChargTimeToFull computeChargeTimeRemaining = " + batteryStats.computeChargeTimeRemaining(j));
            if (batteryStats.computeChargeTimeRemaining(j) != -1) {
            ...
            try {
                j2 = mBatteryStats.computeChargeTimeRemaining() * 1000;
            } catch (Exception unused) {
                Log.d("BatteryInfo", "get chargeTimeRemaining erro");
            }
            Log.d("BatteryInfo", "lesYao chargeTimeRemaining = " + j2);
            return j2;
        }
        Log.d("BatteryInfo", "getChargTimeToFull computeChargeTimeRemaining " + batteryStats.computeChargeTimeRemaining(j));
        if (batteryStats.computeChargeTimeRemaining(j) != -1) {
