...
public class PowerUsageSummary extends PowerUsageBase implements View.OnLongClickListener, BatteryTipPreferenceController.BatteryTipListener {
    ...
    public static final BaseSearchIndexProvider SEARCH_INDEX_DATA_PROVIDER = new BaseSearchIndexProvider(C0019R$xml.power_usage_summary) {
        ...
        public List<String> getNonIndexableKeys(Context context) {
            ...
            if (!OpFeatures.isSupport(new int[]{237}) || OPUtils.isGuestMode()) {
                arrayList.add("wireless_charging_category");
                arrayList.add("reverse_wireless_charging");
                arrayList.add("bed_time_mode_settings");
            }
            if (!OPUtils.isAppExist(context, "com.oneplus.chargingpilar") || Settings.Global.getInt(context.getContentResolver(), "op_stations_installation", 0) != 1) {
                arrayList.add("op_charging_station_setting");
            }
            return arrayList;
        }
    };
    ...
    private void updateChargingStationPref() {
        ...
        if (OPUtils.isAppExist(getContext(), "com.oneplus.chargingpilar")) {
            boolean z = false;
            if (Settings.Global.getInt(getContentResolver(), "op_stations_installation", 0) == 1) {
    ...
    public void refreshUi(int i) {
        if (getContext() != null) {
            ...
            this.mScreenUsagePref.setSummary(OPUtils.handleTimeSummary(StringUtil.formatElapsedTime(getContext(), (double) this.mBatteryUtils.calculateScreenUsageTime(this.mStatsHelper), false)));
        }
    }
