...
public class BatteryEntry {
    ...
    public void getQuickNameIconForUid(int i) {
        ...
        if (sUidCache.containsKey(num)) {
            ...
            String str = uidToDetail.packageName;
            this.defaultPackageName = str;
            this.name = uidToDetail.name;
            Drawable drawable = uidToDetail.icon;
            this.icon = drawable;
            if (str == null && drawable != null) {
                drawable.setTint(this.context.getColor(C0006R$color.op_control_icon_color_active_default));
                return;
            }
            return;
        }
    ...
    public void loadNameAndIcon() {
        ...
        if (this.sipper.uidObj != null) {
            ...
            if (this.name == null) {
                if (uid == -5) {
                    this.name = this.context.getResources().getString(C0017R$string.status_bar_settings_hotspot_title);
                } else {
                    this.name = this.context.getResources().getString(C0017R$string.op_app_already_uninstalled);
                }
            }
            if (this.icon == null) {
                this.icon = packageManager.getDefaultActivityIcon();
