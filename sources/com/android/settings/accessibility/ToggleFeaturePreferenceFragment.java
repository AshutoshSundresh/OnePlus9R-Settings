...
public abstract class ToggleFeaturePreferenceFragment extends SettingsPreferenceFragment implements ShortcutPreference.OnClickCallback {
    ...
    public void onViewCreated(View view, Bundle bundle) {
        ...
        if (this.mImageUri != null) {
            ...
            if (getMetricsCategory() == 1817) {
                animatedImagePreference.setImageRes(C0008R$drawable.op_accessibility_color_reverse);
            } else {
                animatedImagePreference.setImageUri(this.mImageUri);
            }
            animatedImagePreference.setSelectable(false);
            animatedImagePreference.setMaxHeight(screenHeightPixels);
            MagnificationLottie magnificationLottie = new MagnificationLottie(getPrefContext());
            magnificationLottie.setSelectable(false);
            if (getMetricsCategory() != 7) {
                preferenceScreen.addPreference(animatedImagePreference);
            } else {
                preferenceScreen.addPreference(magnificationLottie);
            }
        }
