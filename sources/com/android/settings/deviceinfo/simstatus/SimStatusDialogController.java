...
public class SimStatusDialogController implements LifecycleObserver, OnResume, OnPause, FiveGNetWorkStatus.On5gInfomationUpdateListener {
    ...
    private void updateDataState(int i) {
        ...
        if (i != 0) {
            if (i == 1) {
                ...
                str = i != 3 ? this.mRes.getString(C0017R$string.radioInfo_unknown) : ProductUtils.isUsvMode() ? this.mRes.getString(C0017R$string.connected_to_the_verizon_network_but_data_connection_unavailable) : this.mRes.getString(C0017R$string.radioInfo_data_suspended);
            } else {
                str = ProductUtils.isUsvMode() ? this.mRes.getString(C0017R$string.connected_to_the_verizon_network) : this.mRes.getString(C0017R$string.radioInfo_data_connected);
            }
        } else if (ProductUtils.isUsvMode()) {
            str = this.mRes.getString(C0017R$string.not_connected_to_the_verizon_network);
        } else {
    ...
    private void updateServiceState(ServiceState serviceState) {
        ...
        if (combinedServiceState != 0) {
            if (combinedServiceState == 1 || combinedServiceState == 2) {
                if (ProductUtils.isUsvMode()) {
                    str = this.mRes.getString(C0017R$string.service_does_not_exists_in_this_area);
                } else {
                ...
                str = this.mRes.getString(C0017R$string.service_exists_in_this_area_but_is_not_capable_of_being_used);
            } else {
                str = this.mRes.getString(C0017R$string.radioInfo_service_off);
            }
        } else if (ProductUtils.isUsvMode()) {
            str = this.mRes.getString(C0017R$string.service_exists_in_this_area_and_is_capable_of_being_used);
        } else {
            ...
            if (subscriptionInfo != null) {
                ...
                if (serviceState != null && Utils.isInService(serviceState)) {
                    ...
                    if (isSupport5G() && (fiveGNetWorkStatus = this.mFiveGNetWorkStatus) != null) {
                        ...
                        int i = this.mSlotID;
                        if (defaultDataPhoneId != i || !this.mFiveGNetWorkStatus.getCurrentServiceState(i).getShowFiveGService() || !is5GConnected() || WirelessUtils.isAirplaneModeOn(this.mContext)) {
                        ...
                        String str = "LTE " + this.mRes.getString(C0017R$string.sim_signal_strength, Integer.valueOf(dbm), Integer.valueOf(asuLevel)) + "\nNR " + this.mRes.getString(C0017R$string.sim_signal_strength, Integer.valueOf(rsrp), Integer.valueOf(snr));
                        if (rsrp == 0) {
    ...
    public void On5gInfomationUpdate(int i) {
        ...
        new Handler(Looper.getMainLooper()).post(new Runnable() {
            ...
            public void run() {
                ...
                SimStatusDialogController simStatusDialogController = SimStatusDialogController.this;
                simStatusDialogController.updateSignalStrength(simStatusDialogController.mTempSignalStrength);
            }
        });
    }
    ...
    private void updateNetworkType() {
        ...
        String str = null;
        String networkTypeName2 = dataNetworkType != 0 ? TelephonyManager.getNetworkTypeName(dataNetworkType) : null;
    ...
    private void updateIccidNumber() {
        ...
        if (!z) {
            ...
            this.mDialog.setText(ICCID_INFO_VALUE_ID, String.join(" ", ProductUtils.splitTextToNChar(this.mTelephonyManager.getSimSerialNumber(), 4)));
        } else {
    ...
    private void updateImsRegistrationState() {
        if (ProductUtils.isUsvMode()) {
            this.mDialog.setText(IMS_REGISTRATION_STATE_VALUE_ID, this.mRes.getString(this.mTelephonyManager.isImsRegistered(this.mSubscriptionInfo.getSubscriptionId()) ? C0017R$string.ims_reg_status_registered : C0017R$string.ims_reg_status_not_registered));
        } else if (!isImsRegistrationStateShowUp()) {
    ...
    public PhoneStateListener getPhoneStateListener() {
        return new PhoneStateListener() {
            /* class com.android.settings.deviceinfo.simstatus.SimStatusDialogController.AnonymousClass5 */
