...
public class AccountPreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin, AuthenticatorHelper.OnAccountsUpdateListener, Preference.OnPreferenceClickListener, LifecycleObserver, OnPause, OnResume {
    private static HashMap<String, Drawable> ICONS_CACHE = new HashMap<>();
    ...
    private RestrictedPreference newAddAccountPreference() {
        ...
        restrictedPreference.setIcon(C0008R$drawable.ic_add_24dp);
    ...
    private ArrayList<AccountTypePreference> getAccountTypePreferences(AuthenticatorHelper authenticatorHelper, UserHandle userHandle, ArrayMap<String, AccountTypePreference> arrayMap) {
        ...
        String[] strArr;
        int i3;
        final AuthenticatorHelper authenticatorHelper2 = authenticatorHelper;
        UserHandle userHandle2 = userHandle;
        final String[] enabledAccountTypes = authenticatorHelper.getEnabledAccountTypes();
        final ArrayList<AccountTypePreference> arrayList = new ArrayList<>(enabledAccountTypes.length);
        boolean z = enabledAccountTypes.length > 10;
        if (ICONS_CACHE.isEmpty() && z) {
            OPProgressDialog oPProgressDialog = new OPProgressDialog(this.mContext);
            this.mProgressDialog = oPProgressDialog;
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    for (int i = 0; i < enabledAccountTypes.length; i++) {
                        AccountPreferenceController.ICONS_CACHE.put(enabledAccountTypes[i], authenticatorHelper2.getDrawableForType(((AbstractPreferenceController) AccountPreferenceController.this).mContext, enabledAccountTypes[i]));
                    }
                    new Handler(Looper.getMainLooper()).post(new Runnable() {
                        /* class com.android.settings.accounts.AccountPreferenceController.AnonymousClass1.AnonymousClass1 */

                        public void run() {
                            for (int i = 0; i < arrayList.size(); i++) {
                                AccountTypePreference accountTypePreference = (AccountTypePreference) arrayList.get(i);
                                accountTypePreference.setIcon((Drawable) AccountPreferenceController.ICONS_CACHE.get(accountTypePreference.getAccount().type));
                            }
                            if (AccountPreferenceController.this.mProgressDialog != null && AccountPreferenceController.this.mProgressDialog.isShowing()) {
                                AccountPreferenceController.this.mProgressDialog.dismiss();
                            }
                        }
                    });
                }
        ...
        while (i4 < enabledAccountTypes.length) {
            ...
            if (accountTypeHasAnyRequestedAuthorities(authenticatorHelper2, str) && (labelForType = authenticatorHelper2.getLabelForType(this.mContext, str)) != null) {
                ...
                Drawable drawable = ICONS_CACHE.get(str);
                if (!z && drawable == null) {
                    drawable = authenticatorHelper2.getDrawableForType(this.mContext, str);
                    ICONS_CACHE.put(str, drawable);
                }
                ...
                while (i5 < length) {
                    ...
                    if (remove != null) {
                    ...
                    } else if (AccountRestrictionHelper.showAccount(this.mAuthorities, authenticatorHelper2.getAuthoritiesForAccountType(account.type))) {
                        Bundle bundle = new Bundle();
                        ...
                        i = labelIdForType;
                        arrayList.add(new AccountTypePreference(context, this.mMetricsFeatureProvider.getMetricsCategory(this.mFragment), account, packageForType, labelIdForType, labelForType, AccountDetailDashboardFragment.class.getName(), bundle, drawable));
                        i5 = i2 + 1;
                        authenticatorHelper2 = authenticatorHelper;
                        length = i3;
    ...
    private RestrictedPreference newAddOneplusAccountPreference(Context context) {
        ...
        restrictedPreference.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            /* class com.android.settings.accounts.AccountPreferenceController.AnonymousClass3 */
