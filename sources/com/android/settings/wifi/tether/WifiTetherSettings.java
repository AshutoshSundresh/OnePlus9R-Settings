...
public class WifiTetherSettings extends RestrictedDashboardFragment implements WifiTetherBasePreferenceController.OnTetherConfigUpdateListener, TetherDataObserver.OnTetherDataChangeCallback {
    ...
    public static final BaseSearchIndexProvider SEARCH_INDEX_DATA_PROVIDER = new BaseSearchIndexProvider(C0019R$xml.wifi_tether_settings) {
        /* class com.android.settings.wifi.tether.WifiTetherSettings.AnonymousClass3 */
    ...
    private WifiTetherApChannelPreferenceController mOPWifiTetherApChannelPreferenceController;
    ...
    public void onAttach(Context context) {
        ...
        this.mOPWifiTetherApChannelPreferenceController = (WifiTetherApChannelPreferenceController) use(WifiTetherApChannelPreferenceController.class);
    ...
    public static List<AbstractPreferenceController> buildPreferenceControllers(Context context, WifiTetherBasePreferenceController.OnTetherConfigUpdateListener onTetherConfigUpdateListener) {
        ...
        arrayList.add(new WifiTetherApChannelPreferenceController(context, onTetherConfigUpdateListener));
        return arrayList;
    }

    @Override // com.android.settings.wifi.tether.WifiTetherBasePreferenceController.OnTetherConfigUpdateListener
    public void onTetherConfigUpdated(BasePreferenceController basePreferenceController) {
        final SoftApConfiguration buildNewConfig = buildNewConfig();
        ...
        this.mHandler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                WifiTetherSettings.this.mWifiManager.setSoftApConfiguration(buildNewConfig);
                WifiTetherSettings.this.mOPWifiTetherApChannelPreferenceController.updateDisplay();
            }
        }, 100);
    ...
    private SoftApConfiguration buildNewConfig() {
        ...
        if (!WifiUtils.isSupportDualBand()) {
            i = this.mOPApBandPreferenceController.getBand() == 1 ? 1 : 2;
            builder.setBand(i);
        } else {
            i = this.mApBandPreferenceController.getBandIndex();
        }
        if (securityType == 4 && (i & 8) != 0) {
            builder.setBand(1);
        } else if ((i & 2) != 0) {
            builder.setBand(2);
        }
        int channelIndex = this.mOPWifiTetherApChannelPreferenceController.getChannelIndex();
        if ((i & 2) == 0) {
            if (channelIndex >= 2 && channelIndex <= 13) {
                i2 = channelIndex;
            }
            channelIndex = i2;
        } else if (channelIndex <= 14) {
            channelIndex = 49;
        }
        if ((i & 1) != 0) {
            builder.setChannel(channelIndex, i);
        }
    ...
    private void updateDisplayWithNewConfig() {
        ...
        this.mOPWifiTetherApChannelPreferenceController.updateDisplay();
    }
    ...
    private void checkTetherData() {
        if (OPUtils.isSupportUss() && TetherUtils.getTetherData(getPrefContext()) == 1) {
            ...
            if (wifiTetherSwitchBarController != null) {
                wifiTetherSwitchBarController.stopTether();
            }
        }
    }
}
