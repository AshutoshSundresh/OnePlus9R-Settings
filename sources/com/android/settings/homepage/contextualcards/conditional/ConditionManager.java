...
public class ConditionManager {
    ...
    private final WeakReference<ConditionListener> mListener;

    public ConditionManager(Context context, ConditionListener conditionListener) {
        this.mAppContext = context.getApplicationContext();
        this.mListener = new WeakReference<>(conditionListener);
        initCandidates();
    }
    ...
    public List<ContextualCard> getDisplayableCards() {
        ...
        for (Future future : arrayList2) {
            try {
                ContextualCard contextualCard = (ContextualCard) future.get(80, TimeUnit.MILLISECONDS);
    ...
    public void onConditionChanged() {
        WeakReference<ConditionListener> weakReference = this.mListener;
        if (weakReference != null && weakReference.get() != null) {
            this.mListener.get().onConditionsChanged();
        }
    }
