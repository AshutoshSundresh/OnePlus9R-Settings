...
public class TopLevelSettings extends DashboardFragment implements PreferenceFragmentCompat.OnPreferenceStartFragmentCallback, OPOnlineConfigManager.OnSupportConfigCompleteParseListener {
    ...
    public void OnSupportConfigParseCompleted() {
        ...
        Preference findPreference2 = findPreference(((SupportPreferenceController) use(SupportPreferenceController.class)).getPreferenceKey());
        if (findPreference2 != null) {
            findPreference2.setVisible(OPOnlineConfigManager.isSupportEnable());
            if (OPOnlineConfigManager.isSupportEnable() && (findPreference = findPreference("top_level_about_device")) != null) {
    ...
    public boolean shouldForceRoundedIcon() {
        return getPrefContext().getResources().getBoolean(C0005R$bool.config_force_rounded_icon_TopLevelSettings);
    }
}
