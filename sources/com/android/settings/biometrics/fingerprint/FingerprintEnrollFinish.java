...
public class FingerprintEnrollFinish extends BiometricEnrollBase {
    ...
    public void onCreate(Bundle bundle) {
        ...
        if (((FingerprintManager) getSystemService("fingerprint")).getEnrolledFingerprints(this.mUserId).size() >= getResources().getInteger(17694824)) {
            this.mBtnAdd.setVisibility(4);
        }
        adjustTitleSize();
        OPUtils.setLightNavigationBar(getWindow(), 1);
    ...
    public void onResume() {
        ...
        boolean z = false;
        if (fingerprintManagerOrNull != null && fingerprintManagerOrNull.getEnrolledFingerprints(this.mUserId).size() >= getResources().getInteger(17694824)) {
            z = true;
        }
        if (z) {
            this.mBtnAdd.setVisibility(4);
        ...
        updateFpBlackGestureSettings();
    }

    private void updateFpBlackGestureSettings() {
        FingerprintManager fingerprintManagerOrNull = Utils.getFingerprintManagerOrNull(this);
        if (fingerprintManagerOrNull != null) {
            int size = fingerprintManagerOrNull.getEnrolledFingerprints(this.mUserId).size();
            if (OPUtils.isSupportCustomFingerprint() && size == 1) {
                Log.d("FingerprintEnrollFinish", "updateFpBlackGestureSettings: turn on black gesture");
                OPGestureUtils.set1(getApplicationContext(), 15);
            }
        }
    }
    ...
    private void adjustTitleSize() {
        if (OPUtils.isLargerFontSize(this) && OPUtils.isLargerScreenZoom(this)) {
            ((TextView) findViewById(C0010R$id.message)).setTextSize(18.0f);
            ((TextView) findViewById(C0010R$id.message_secondary)).setTextSize(0, 48.0f);
        }
    }
}
