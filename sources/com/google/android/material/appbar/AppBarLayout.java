...
public class AppBarLayout extends LinearLayout implements CoordinatorLayout.AttachedBehavior {
    ...
    public void setContentVelocity(int i) {
        View findFirstScrollingBehaviorChild;
        if ((getParent() instanceof CoordinatorLayout) && (findFirstScrollingBehaviorChild = findFirstScrollingBehaviorChild((CoordinatorLayout) getParent())) != null) {
            CoordinatorLayout.Behavior behavior = ((CoordinatorLayout.LayoutParams) findFirstScrollingBehaviorChild.getLayoutParams()).getBehavior();
            if (behavior instanceof BaseBehavior) {
                ((BaseBehavior) behavior).setContentVelocity(i);
            }
        }
    }

    private View findFirstScrollingBehaviorChild(CoordinatorLayout coordinatorLayout) {
        int childCount = coordinatorLayout.getChildCount();
        for (int i = 0; i < childCount; i++) {
            View childAt = coordinatorLayout.getChildAt(i);
            if (((CoordinatorLayout.LayoutParams) childAt.getLayoutParams()).getBehavior() instanceof BaseBehavior) {
                return childAt;
            }
        }
        return null;
