...
public abstract class DefaultBasePreferenceController extends DefaultAppPreferenceController {
    ...
    public Drawable getDefaultAppIcon() {
        if (!isAvailable()) {
            return this.mContext.getResources().getDrawable(C0008R$drawable.ic_remove_circle);
        }
        DefaultAppInfo defaultAppInfo = getDefaultAppInfo();
        if (defaultAppInfo != null) {
            return defaultAppInfo.loadIcon();
        }
        Drawable onlyAppIcon = getOnlyAppIcon();
        return onlyAppIcon == null ? this.mContext.getResources().getDrawable(C0008R$drawable.ic_remove_circle) : onlyAppIcon;
    }
