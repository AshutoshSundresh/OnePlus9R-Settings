...
public class OPOnlineConfigManager {
    ...
    private static List<String> mLocalVideoSpWhitelist;
    private static List<String> mVideoSpWhitelist = new ArrayList();
    ...
    private Handler mHandler = new Handler(this.mHandlerThread.getLooper()) {
        ...
        public void handleMessage(Message message) {
            ...
            if (i == 1) {
                ...
                OPOnlineConfigManager.this.parseSlaConfigFromJson(new ConfigGrabber(OPOnlineConfigManager.this.mContext, "WhiteListVideoSPConfig").grabConfig());
            }
        }
    };
    ...
    private ConfigObserver mWhitelistSPConfigObserver;
    ...
    private OPOnlineConfigManager(Context context) {
        ...
        mLocalVideoSpWhitelist = Arrays.asList(context.getResources().getStringArray(C0003R$array.op_local_video_sp_white_list_app));
    }
        ...
        if (jSONArray != null) {
            for (int i = 0; i < jSONArray.length(); i++) {
                try {
                    ...
                    if (jSONObject != null && jSONObject.has("name") && jSONObject.getString("name").equals("SlaDownloadWhiteList")) {
    ...
    public void init() {
        ...
        if (OPUtils.isSupportViewSR()) {
            ConfigObserver configObserver3 = new ConfigObserver(this.mContext, this.mHandler, new WhitelistVideoSPConfigUpdater(), "WhiteListVideoSPConfig");
            this.mWhitelistSPConfigObserver = configObserver3;
            configObserver3.register();
            this.mHandler.sendEmptyMessageDelayed(3, 100);
        }
    }

    /* access modifiers changed from: package-private */
    public class WhitelistVideoSPConfigUpdater implements ConfigObserver.ConfigUpdater {
        WhitelistVideoSPConfigUpdater() {
        }

        public void updateConfig(JSONArray jSONArray) {
            OPOnlineConfigManager.this.resolveVSPConfigFromJSON(jSONArray);
        }
    }

    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void resolveVSPConfigFromJSON(JSONArray jSONArray) {
        if (jSONArray == null) {
            String string = OPPrefUtil.getString("WhiteListVideoSPConfig", null);
            if (Build.DEBUG_ONEPLUS) {
                Log.d("OPOnlineConfigManager", "get SP Online Config settings parseSlaConfigFromJson jsonArray is null,use old json:" + string);
            }
            if (string != null) {
                try {
                    jSONArray = new JSONArray(string);
                } catch (JSONException e) {
                    Log.e("OPOnlineConfigManager", "get SP Online Config settings parseSlaConfigFromJson JSONException:" + e.getMessage());
                    e.printStackTrace();
                }
            } else {
                Log.d("OPOnlineConfigManager", "get SP Online Config settings parseSlaConfigFromJson jsonArray is null, return");
                return;
            }
        }
        if (Build.DEBUG_ONEPLUS && jSONArray != null) {
        ...
        try {
            synchronized (mVideoSpWhitelist) {
                OPPrefUtil.putString("WhiteListVideoSPConfig", jSONArray.toString());
                mVideoSpWhitelist.clear();
                for (int i = 0; i < jSONArray.length(); i++) {
                    JSONObject jSONObject = jSONArray.getJSONObject(i);
                    if (jSONObject != null && jSONObject.has("name")) {
                        String string2 = jSONObject.getString("name");
                        mVideoSpWhitelist.add(string2);
                        if (Build.DEBUG_ONEPLUS) {
                            Log.d("OPOnlineConfigManager", "resolveVSPConfigFromJSON " + string2);
                        }
                    }
                }
            }
        } catch (JSONException e2) {
            Log.w("OPOnlineConfigManager", "resolveVSPConfigFromJSON JSONException:", e2);
        } catch (Exception e3) {
            Log.w("OPOnlineConfigManager", "resolveVSPConfigFromJSON error:", e3);
        }
    }

    public static List<String> getVideoSpWhitelist() {
        synchronized (mVideoSpWhitelist) {
            if (mVideoSpWhitelist.isEmpty()) {
                return mLocalVideoSpWhitelist;
            }
            return mVideoSpWhitelist;
        }
    }
    ...
    private void parseConfigFromJson(JSONArray jSONArray) {
        ...
        for (int i = 0; i < jSONArray.length(); i++) {
            try {
                JSONObject jSONObject = jSONArray.getJSONObject(i);
                if (jSONObject != null && jSONObject.has("name") && jSONObject.getString("name").equals("op_multiapp_white_list_p")) {
                    JSONArray jSONArray2 = jSONObject.getJSONArray("value");
                ...
                if (jSONObject != null && jSONObject.has("need_show")) {
