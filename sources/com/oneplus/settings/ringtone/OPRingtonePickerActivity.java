...
public class OPRingtonePickerActivity extends OPRingtoneBaseActivity implements Preference.OnPreferenceClickListener {
    ...
    private void initPreference(boolean z) {
        if (this.mSystemRings == null) {
            ...
            if (cursor != null && cursor.moveToFirst()) {
                ...
                do {
                    ...
                    String replaceWith = OPNotificationUtils.replaceWith(this, this.mCursor.getString(1), settingForType);
                    oPRadioButtonPreference.setTitle(replaceWith);
                    Uri uriFromCursor = OPRingtoneManager.getUriFromCursor(this.mCursor);
                    oPRadioButtonPreference.setKey(uriFromCursor.toString());
                    if (OPUtils.isSupportSortByTrack()) {
                        String defaultRingtoneFileName = OPRingtoneManager.getDefaultRingtoneFileName(this, settingForType);
                        if (!TextUtils.isEmpty(defaultRingtoneFileName) && defaultRingtoneFileName.contains(".")) {
                            defaultRingtoneFileName = defaultRingtoneFileName.substring(0, defaultRingtoneFileName.lastIndexOf(".")).replace("_", " ");
                        }
                        if (TextUtils.isEmpty(replaceWith) || !TextUtils.equals(replaceWith.toLowerCase(), defaultRingtoneFileName)) {
                            i++;
                            oPRadioButtonPreference.setOrder(10 + i);
                        } else {
                            oPRadioButtonPreference.setOrder(10);
                        }
                    }
                    oPRadioButtonPreference.setOnPreferenceClickListener(this);
        ...
        for (int i2 = 0; i2 < this.mSystemRings.size(); i2++) {
            if (z) {
                getPreferenceScreen().addPreference(this.mSystemRings.get(i2));
            } else {
                getPreferenceScreen().removePreference(this.mSystemRings.get(i2));
            }
        }
        if (z) {
    ...
    public boolean onPreferenceClick(Preference preference) {
        ...
        if (key.equals("setting_key")) {
            ...
            Log.d("OPRingtonePickerActivity", "key = " + key + "mUriForDefaultItem = " + this.mUriForDefaultItem);
    ...
    private void updateChecks(String str) {
        ...
        if (list != null) {
            for (OPRadioButtonPreference oPRadioButtonPreference : list) {
                Log.d("OPRingtonePickerActivity", "key = " + str + "p.getKey() = " + oPRadioButtonPreference.getKey());
                oPRadioButtonPreference.setChecked(keyEquals(str, oPRadioButtonPreference.getKey()));
            }
        }
    }
    ...
    private void updateLocalRingtone() {
        Uri uri;
        Uri actualDefaultRingtoneUri = RingtoneManager.getActualDefaultRingtoneUri(getBaseContext(), this.mType);
        if (actualDefaultRingtoneUri != null && (uri = this.mUriForDefaultItem) != null && !uri.getPath().equals(actualDefaultRingtoneUri.getPath())) {
            this.mUriForDefaultItem = RingtoneManager.getActualDefaultRingtoneUri(getBaseContext(), this.mType);
            updateSelected();
        }
    }
