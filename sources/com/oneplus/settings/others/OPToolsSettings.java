...
import com.oneplus.settings.worklifebalance.OPWLBBannerPrefController;
import com.oneplus.settings.worklifebalance.OPWLBBannerPreference;
...
public class OPToolsSettings extends SettingsPreferenceFragment implements Preference.OnPreferenceChangeListener, Preference.OnPreferenceClickListener {
    ...
    private Preference mSmartSideBarPreference;
    ...
    private OPWLBBannerPreference mWorkLifeBalancePreference;
    ...
    private void updateView() {
        OPWLBBannerPreference oPWLBBannerPreference;
        ...
        this.mWorkLifeBalancePreference = (OPWLBBannerPreference) findPreference("worklifebalance");
        this.mSmartSideBarPreference = findPreference("op_smart_side_bar");
        if (!OpFeatures.isSupport(new int[]{329})) {
            this.mSmartSideBarPreference.setVisible(false);
        }
        OPWLBBannerPrefController oPWLBBannerPrefController = new OPWLBBannerPrefController(getContext(), this.mWorkLifeBalancePreference);
        Lifecycle settingsLifecycle = getSettingsLifecycle();
        if (settingsLifecycle != null) {
            settingsLifecycle.addObserver(oPWLBBannerPrefController);
        }
        if (OPUtils.isSupportQuickLaunch()) {
        ...
        if (!OpFeatures.isSupport(new int[]{61})) {
        ...
        if (!OPUtils.isAppExist(getActivity(), "com.oneplus.opwlb") && (oPWLBBannerPreference = this.mWorkLifeBalancePreference) != null) {
            preferenceScreen.removePreference(oPWLBBannerPreference);
        }
        if (this.mWlbFeatureObserver == null) {
