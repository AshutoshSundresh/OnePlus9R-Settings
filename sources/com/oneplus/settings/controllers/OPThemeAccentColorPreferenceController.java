...
public class OPThemeAccentColorPreferenceController extends BasePreferenceController {
    ...
    public CharSequence getSummary() {
        ...
        if (TextUtils.isEmpty((CharSequence) hashMap.get(string.toLowerCase()))) {
            str = this.mContext.getString(C0017R$string.customization_settings_title);
        } else {
            str = (String) hashMap.get(string.toLowerCase());
        }
        if (OPThemeUtils.isSupportREDTheme() && OPThemeUtils.isCurrentREDTheme(this.mContext)) {
            str = this.mContext.getString(C0017R$string.op_control_red_theme_accent_color_summary) + this.mContext.getString(C0017R$string.op_red_theme_accent_color_disable_tips);
        }
        return (Settings.System.getInt(this.mContext.getContentResolver(), "oem_white_mode_accent_color_index", -2) == -1 || Settings.System.getInt(this.mContext.getContentResolver(), "oem_black_mode_accent_color_indexq", -2) == -1) ? this.mContext.getString(C0017R$string.customization_settings_title) : str;
    }
