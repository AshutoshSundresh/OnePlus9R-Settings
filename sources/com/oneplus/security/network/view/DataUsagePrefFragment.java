...
public class DataUsagePrefFragment extends PreferenceFragmentCompat {
    private Preference hotspotPref;
    ...
    public void onCreate(Bundle bundle) {
        ...
        this.hotspotPref = findPreference("op_hotspot_usage");
        if (!FunctionUtils.isUsvMode() && this.hotspotPref != null) {
            Preference findPreference = findPreference("screen_brightness");
            Objects.requireNonNull(findPreference);
            ((PreferenceCategory) findPreference).removePreference(this.hotspotPref);
        }
        Preference findPreference2 = findPreference("op_data_usage_package_set");
        this.mDataUsagePackageSet = findPreference2;
        ...
        initUIState();
        if (this.hotspotPref != null && FunctionUtils.isUsvMode()) {
            this.hotspotPref.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
                ...
                public boolean onPreferenceClick(Preference preference) {
                    ...
                    int i;
                    try {
                        Intent intent = new Intent("android.oneplus.action.DATAUSAGE_DATAUSAGELIS");
                        if (!Utils.isIntentReceiverExists(DataUsagePrefFragment.this.getActivity(), intent)) {
                            Log.w("DataUsagePrefFragment", "replace a new action");
                            intent = new Intent("android.settings.MOBILE_DATA_USAGE");
                        }
                        if (DataUsagePrefFragment.this.mNeedHeadView) {
                            i = OPSNSUtils.findSubIdBySlotId(DataUsagePrefFragment.this.mCurrentSlotId);
                            networkTemplate = NativeTrafficDataModel.getNetworkTemplate(i);
                        } else {
                            networkTemplate = NetworkTemplate.buildTemplateWifiWildcard();
                            i = 0;
                        }
                        intent.putExtra(":settings:show_fragment_title", DataUsagePrefFragment.this.hotspotPref.getTitle());
                        intent.putExtra("sub_id", i);
                        intent.putExtra("network_template", (Parcelable) networkTemplate);
                        intent.putExtra("is_hotspot", true);
                        DataUsagePrefFragment.this.startActivity(intent);
                    } catch (Exception e) {
    ...
    public void animateUpdateMonthlyRemainingData(final long j, final long j2) {
        if (getActivity() == null || !isAdded()) {
            this.mHandler.post(new Runnable() {
                /* class com.oneplus.security.network.view.DataUsagePrefFragment.AnonymousClass3 */
