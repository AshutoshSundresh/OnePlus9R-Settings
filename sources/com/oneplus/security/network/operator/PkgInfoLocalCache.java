...
public class PkgInfoLocalCache extends BaseSharePreference {
    ...
    public static void clearPkgUsedMonthlyLocalCache(Context context, int i) {
        synchronized (mLock) {
            SharedPreferences.Editor edit = BaseSharePreference.getDefaultSharedPreferences("key_pkg_info_local_cache").edit();
            edit.remove("key_pkg_used_this_month_subid_" + OPSNSUtils.findSubIdBySlotId(i));
            Log.d("PkgInfoLocalCache", "clearPkgUsedMonthlyLocalCache slot id is " + i);
            edit.apply();
        }
    }
